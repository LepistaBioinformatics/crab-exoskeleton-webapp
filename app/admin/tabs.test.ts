...
import { AGENT_TABS, DEFAULT_TAB, SCOPED_TABS, TAB_KEYS, isScopedTab, parseTab } from "./tabs";
...
describe("SCOPED_TABS", () => {
  it("is every tab except Branding, which has no scope", () => {
    expect(SCOPED_TABS).toEqual(["files", "secrets", "skills", "model", "members"]);
    expect(SCOPED_TABS).not.toContain("branding");
  });

  it("together with Branding accounts for every tab, so the two-level nav has no gap", () => {
    expect([...SCOPED_TABS, "branding"].sort()).toEqual([...TAB_KEYS].sort());
  });

  it("agrees with isScopedTab", () => {
    for (const t of TAB_KEYS) {
      expect(isScopedTab(t)).toBe(SCOPED_TABS.includes(t));
    }
  });
});
