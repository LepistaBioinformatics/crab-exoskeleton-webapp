...
// The secrets pane: a stack of collapsible groups, the model first and then one
// group per secret sink.
//
// It used to be a fixed overlay drawer behind its own header icon. It is now the
// fifth section of the right sidebar (right-rail-discoverability FR-3) — same
// content, reached the same way as memory, the graph, tasks and files, and carried
// in the URL like them. Mounting IS opening: the sidebar renders this only for the
// chosen section, so the `open` prop and its guards are gone.
//
...
const WRITABLE: SecretFormat[] = USER_SECRET_FORMATS.filter(
  (f) => f !== "file",
);
...
export default function SecretsSection({
  ...
  // change is stored but not yet live. The restart banner carries the button; this
  // is the confirmation at the point of action, so the member is not left wondering
  ...
  async function onDelete(fmt: SecretFormat, secretName: string) {
    if (!window.confirm(t.secrets.deleteConfirm.replace("{name}", secretName)))
      return;
    setBusy(secretName);
    setLoadError(null);
    try {
  ...
  return (
    <div className="flex-1 overflow-auto px-4 py-4">
      ...
      <div className="flex flex-col">
        ...
        {/* LAST, and closed like every group above it.
            It used to come first and open itself, on the reasoning that a broken model
            is the more urgent of the two questions this pane answers. In use that was
            backwards: the pane opened on a form about models with the keys pushed below
            the fold, whichever question the member had arrived with. The pane is called
            Secrets — the sinks come first, and nothing decides for the member what they
            came for.

            Mounted with the pane, so leaving the section does not leave a poll or a
            half-filled key field alive off screen. */}
        <OwnModelsSection workspace={workspace} onChanged={onRestartNeeded ?? (() => {})} />
      </div>
    </div>
  );
}
