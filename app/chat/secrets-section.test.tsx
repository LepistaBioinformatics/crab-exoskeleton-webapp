...
// find anything. It is now the fifth of the workspace's sections, reached exactly the
// way the other four are: a row in the sidebar and a screen in the centre pane.
...
async function mount(section: "secrets" | "memory") {
...
describe("secrets as a section", () => {
  it("renders in the centre pane when the section is the destination", async () => {
    ...
    // The section's own copy, not the drawer's chrome: the fixed panel and its
    // backdrop are gone.
  ...
  // A destination is mounted only while it IS the screen, which is what makes the
  // laziness structural rather than a guard somebody has to remember: standing in
  // another section, nothing here has been asked for.
  it("asks the proxy for nothing while another section is the destination", async () => {
    const host = await mount("memory");
    expect(host.textContent).not.toContain(t.secrets.savedForYou);
