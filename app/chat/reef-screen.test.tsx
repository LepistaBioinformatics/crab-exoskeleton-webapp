...
// The reef tab. What this suite asserts is the three distinctions the screen
...
vi.mock("@/lib/reef", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/reef")>();
  return {
    ...actual,
...
import { ReefError } from "@/lib/reef";
...
async function render() {
  ...
  await act(async () => {
    root!.render(<ReefScreen workspace={workspace} />);
  });
...
describe("the reef tab", () => {
  it("renders NOTHING when the operator never enabled the reef", async () => {
    readTimeline.mockRejectedValue(new ReefError("reef_off"));
    readCapabilities.mockRejectedValue(new ReefError("reef_off"));
  ...
  it("says nothing is here yet, rather than reporting a failure", async () => {
    ...
    const html = await render();
    expect(html).toContain(en.reef.none);
    expect(html).not.toContain(en.reef.unreachable);
    expect(html).not.toContain(en.reef.loadFailed);
  });

  it("reports an unreachable reef as its own state, with a retry", async () => {
    readTimeline.mockRejectedValue(new ReefError("reef_unreachable"));
    readCapabilities.mockRejectedValue(new ReefError("reef_unreachable"));

    const html = await render();
    expect(html).toContain(en.reef.unreachable);
    expect(html).toContain(en.reef.retry);
    // "Nothing shared yet" would be a lie here.
    expect(html).not.toContain(en.reef.none);
  });
  ...
  it("does not offer the pending reading to somebody who governs nothing", async () => {
    ...
    const html = await render();
    expect(html).toContain(en.reef.received);
    expect(html).toContain(en.reef.published);
    // ABSENT, not disabled.
    expect(html).not.toContain(en.reef.pending);
  });
  ...
  it("offers it to a governing role", async () => {
    ...
    const html = await render();
    expect(html).toContain(en.reef.pending);
  });
  ...
  it("shows a directly shared item as HELD, with a way to admit it", async () => {
      ...
      held: [
        {
          ...
          from: "reef:actor:bob:service",
          object: { id: "reef:obj:1", type: "MemoryNote", cell: "soil-ph", content: "6.4" },
    ...
    const html = await render();
    expect(html).toContain(en.reef.heldTitle);
    expect(html).toContain(en.reef.heldHint);
    expect(html).toContain(en.reef.admit);
